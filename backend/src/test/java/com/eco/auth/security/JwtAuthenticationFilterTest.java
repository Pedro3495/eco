...
class JwtAuthenticationFilterTest {
    ...
    @Test
    void shouldAuthenticateUserWhenAccessTokenCookieIsValid() throws ServletException, IOException {
        UUID userId = UUID.randomUUID();
        User user = new User("Usuario Dev", "dev@eco.com", "hash");
        JwtAuthenticationFilter filter = new JwtAuthenticationFilter(jwtService, userRepository);
        MockHttpServletRequest request = new MockHttpServletRequest();
        ...
        request.setCookies(new Cookie("eco_access_token", "cookie-access-token"));
        when(jwtService.validateAndExtractUserId("cookie-access-token")).thenReturn(userId);
        when(userRepository.findById(userId)).thenReturn(Optional.of(user));
